...
interface ReceiptProps {
  ...
  /** Matches `data-print-doc` for selective @media print and printDocument(). */
  printDoc?: string;
}
...
export function Receipt({ order, variant = "receipt", printDoc }: ReceiptProps) {
  ...
  return (
    <div
      ...
      {...(printDoc ? { "data-print-doc": printDoc } : {})}
      style={{
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: "28px", padding: "14px 16px", background: "#f8fafc", borderLeft: `3px solid ${ACCENT}`, fontSize: "10.5px", color: MUTED, lineHeight: 1.6 }}>
          ...
          {isReceipt
            ? `This receipt confirms payment for the items listed above. Please retain it for your records. For returns or questions, contact ${brand.supportEmail} within 48 hours.`
            : isQuotation
              ? `This quotation is valid for 14 days from the date of issue. Prices and availability are indicative and subject to confirmation. This document is not a demand for payment. For questions, contact ${brand.salesEmail}.`
              : isProforma
                ? `This proforma invoice is provided for planning purposes only and does not constitute a tax invoice or a confirmation of payment. Final pricing is confirmed upon order acceptance by ${brand.legalName}. For questions, contact ${brand.salesEmail}.`
                : `Payment due within 7 days of issue. Goods remain the property of ${brand.legalName} until paid in full. For questions about this invoice, contact ${brand.billingEmail}.`}
        </div>
      </div>
      ...
      >
        ...
        <div>
          ...
          <div style={{ opacity: 0.85 }}>{brand.supportEmail}</div>
          <div style={{ opacity: 0.85 }}>{brand.website}</div>
        </div>
      ...
      <div style={{ textAlign: "center", padding: "10px", fontSize: "9px", color: MUTED, letterSpacing: "1px" }}>
        {brand.legalName} · Connecting farmers, retailers and logistics across the country
      </div>
    </div>
  );
}
