...
export default function AdminPricing() {
  ...
  const [sspDraft, setSspDraft] = useState(String(rates.usdToSsp));
  const [usgDraft, setUsgDraft] = useState(String(rates.usdToUsg));

  useEffect(() => {
    setSspDraft(String(rates.usdToSsp));
    setUsgDraft(String(rates.usdToUsg));
  }, [rates.usdToSsp, rates.usdToUsg]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {t("Used for SSP/USG price conversion and checkout", "تُستخدم لتحويل أسعار SSP/USG والدفع")}
              </p>
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <label className="text-sm font-medium">{t("USD → SSP", "دولار → جنيه")}</label>
              <div className="flex items-center gap-2">
                <Input
                  type="number"
                  step="0.01"
                  value={sspDraft}
                  onChange={e => setSspDraft(e.target.value)}
                  data-testid="input-exchange-rate-ssp"
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">{t("USD → USG", "دولار → شلن أوغندي")}</label>
              <div className="flex items-center gap-2">
                <Input
                  type="number"
                  step="0.01"
                  value={usgDraft}
                  onChange={e => setUsgDraft(e.target.value)}
                  data-testid="input-exchange-rate-usg"
                  className="max-w-[200px]"
                />
                <span className="text-sm text-muted-foreground">USG / USD</span>
              </div>
            </div>
          </div>
          <Button
            size="sm"
            onClick={async () => {
              const usdToSsp = parseFloat(sspDraft);
              const usdToUsg = parseFloat(usgDraft);
              if (!Number.isFinite(usdToSsp) || usdToSsp <= 0 || !Number.isFinite(usdToUsg) || usdToUsg <= 0) {
                toast({ title: t("Invalid rate", "سعر غير صالح"), variant: "destructive" });
                return;
              }
              try {
                await updateRates({ usdToSsp, usdToUsg });
                toast({ title: t("Exchange rates updated", "تم تحديث أسعار الصرف") });
              } catch {
                toast({ title: t("Failed to save rates", "فشل حفظ الأسعار"), variant: "destructive" });
              }
            }}
            data-testid="button-save-rate"
          >
            {t("Save rates", "حفظ الأسعار")}
          </Button>
          <p className={`text-xs text-muted-foreground ${currenciesInlineClassName}`}>{currenciesDisplay()}</p>
        </div>
