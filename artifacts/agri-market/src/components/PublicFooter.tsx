      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 grid grid-cols-1 sm:grid-cols-3 gap-8">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-extrabold text-lg">AgriMarket</span>
          </div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li className="flex items-center gap-2"><Mail className="w-4 h-4 text-primary" /> hello@agrimarket.ss</li>
            <li className="flex items-center gap-2"><MapPin className="w-4 h-4 text-primary" /> Juba, South Sudan</li>
          </ul>
        </div>
      </div>
      <div className="border-t border-border/60 py-5 text-center text-xs text-muted-foreground">
        © {new Date().getFullYear()} AgriMarket — {t("Powering South Sudan's agricultural trade", "تمكين التجارة الزراعية في جنوب السودان")}
      </div>
    </footer>
  );
}
